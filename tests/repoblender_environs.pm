...
EOL
    assert_script_run($_, timeout => 900) foreach (split /\n/, $environs_test);
    upload_logs("test-environs.log")  if -f "test-environs.log";
    upload_logs("/var/log/apache2/error_log") if -f "/var/log/apache2/error_log";
    wait_still_screen;
    save_screenshot;
}
